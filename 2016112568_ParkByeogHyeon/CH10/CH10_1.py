...
class Student:
    ...
    def calc_grade(self,m_index,e_index,k_index):
        math_index=m_index+1
        english_index=e_index+1
        korean_index=k_index+1
        student_num=len(students)
        grade=[]
        if math_index/student_num<=0.3:
            grade.append("A")
        elif math_index/student_num<=0.7:
            grade.append("B")
        else:
            grade.append("C")
        if english_index/student_num<=0.3:
            grade.append("A")
        elif english_index/student_num<=0.7:
            grade.append("B")
        else:
            grade.append("C")
        if korean_index/student_num<=0.3:
            grade.append("A")
        elif korean_index/student_num<=0.7:
            grade.append("B")
        else:
            grade.append("C")
        return grade
